...
gridOUTPUT_layers= NaN( voxcountX,voxcountY,voxcountZ,numel(raydirection) );
countdirections = 0;

if strfind(raydirection,'x')
  countdirections = countdirections + 1;
  [save1,bb1]=VOXELISEinternal(gridCOy,gridCOz,gridCOx,meshXYZ(:,[2,3,1],:));
  gridOUTPUT(:,:,:,countdirections) = permute( save1,[3,1,2] );
end

if strfind(raydirection,'y')
  countdirections = countdirections + 1;
  [save2,bb2]=VOXELISEinternal(gridCOz,gridCOx,gridCOy,meshXYZ(:,[3,1,2],:)) ;
  gridOUTPUT(:,:,:,countdirections) = permute( save2,[2,3,1] );
end

if strfind(raydirection,'z')
  countdirections = countdirections + 1;
  [save3,bb3]=VOXELISEinternal(gridCOx,gridCOy,gridCOz,meshXYZ);
  gridOUTPUT(:,:,:,countdirections) = save3;
end
...
%==========================================================================
function [gridOUTPUT,bb] = VOXELISEinternal(gridCOx,gridCOy,gridCOz,meshXYZ)
...
bb = NaN(voxcountX,voxcountY,voxcountZ);
      ...
      if isempty(possibleCROSSLIST)==0  %Only continue the analysis if some nearby facets were actually identified
        ...
        if rem(numel(gridCOzCROSS),2)==0  % Only rays which cross an even number of facets are voxelised
          ...
          for loopASSIGN = 1:(numel(gridCOzCROSS)/2)
            ...
            voxelSEGMENTS=find(voxelsINSIDE);
            a=diff(voxelSEGMENTS);
            b=find([a inf]>1);
            c=diff([0 b]); %length of the sequences
            %lengths=zeros(size(voxelsINSIDE));
            %lengths(voxelSEGMENTS(1:c))=c(i);
            gridOUTPUT(loopX,loopY,voxelsINSIDE) = 1;
            bb(loopX,loopY,voxelsINSIDE) = c;
          end %for
